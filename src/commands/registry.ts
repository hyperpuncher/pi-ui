...
export const appCommands: AppCommand[] = [
	{
		...
		action: "@post('/sessions/new', { filterSignals: { include: /^$/ } }); requestAnimationFrame(() => document.getElementById('prompt-input')?.focus())",
	},
	{
		...
		action: "@post('/sessions/new-temporary', { filterSignals: { include: /^$/ } }); requestAnimationFrame(() => document.getElementById('prompt-input')?.focus())",
	},
	{
		id: "resume-session",
		...
		action: "document.getElementById('session-dialog')?.showModal(); @post('/sessions/list', { filterSignals: { include: /^$/ } }); requestAnimationFrame(() => document.getElementById('session-input')?.focus())",
	},
	{
		id: "session-tree",
		title: "Session tree",
	...
	},
	{
		...
		shortcut: { display: "", native: "", keys: [] },
		action: "document.getElementById('command-dialog')?.close(); @post('/auth/open-login', { filterSignals: { include: /^$/ } })",
	},
	{
		id: "logout",
		title: "Log out",
		...
		shortcut: { display: "", native: "", keys: [] },
		action: "document.getElementById('command-dialog')?.close(); @post('/auth/open-logout', { filterSignals: { include: /^$/ } })",
	},
	{
		id: "change-workspace",
